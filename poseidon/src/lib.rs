...
impl<F: Field + From<BigUint>> Poseidon<F> {
    ...
    pub fn arc(&mut self,ith_round: usize) {
        ...
        let constant = &constant[ith_round * self.state.len()..ith_round * self.state.len() + self.state.len()];
        self.state
            .iter_mut()
            .zip(constant.iter())
    ...
    pub fn permutation(&mut self, input_data: &[F]) -> Result<F,String> {
        if input_data.len() < 1 || input_data.len() > self.mds.last().unwrap().last().unwrap().len() -1 {
                return Err(format!("the length of input data cannot less than 1 and great than {}",self.mds.last().unwrap().last().unwrap().len() -1));
        } 
...
mod tests {
    ...
    fn poseidon_2() {
        ...
        assert_eq!(r.to_string(), "7853200120776062878684798364095072458815029376092732009249414926327459813530".to_string())
    }
    ...
    fn poseidon_4() {
        ...
        assert_eq!(r.to_string(), "18821383157269793795438455681495246036402687001665670618754263018637548127333".to_string())
    }
}
