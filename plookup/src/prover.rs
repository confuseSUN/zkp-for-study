...
pub(super) fn prove(
    ...
    let f_poly = Poly::from_coefficients_vec(f_coefs);
    ...
    let h1_poly = Poly::from_coefficients_vec(h1_coefs);
    ...
    let h2_poly = Poly::from_coefficients_vec(h2_coefs);
    ...
    assert!(r_poly.evaluate(&zeta) == Scalar::zero());
